...
Covers both entry points: from raw tracker keypoints
(``calculate_center_of_mass_per_frame``) and from already standard-human-named
positions (``calculate_center_of_mass`` — the path used by the rigidified
...
def test_com_from_standard_human_matches_tracker_path():
    # The refactor must be behaviour-preserving: feeding standard-human-named
    # positions directly equals mapping tracker keypoints then computing CoM.
    bio = load_body_biomechanics()
    pose = _upright_rtmpose_pose()

    via_tracker = calculate_center_of_mass_per_frame(pose, bio)
    standard_human_positions = bio.tracker_mapping.apply(pose)
    via_standard_human = calculate_center_of_mass(standard_human_positions, bio)

    assert np.allclose(via_tracker.total_body_com, via_standard_human.total_body_com)
    assert via_standard_human.confidence == via_tracker.confidence
...
def test_thigh_com_is_com_fraction_along_the_segment():
    ...
    bio = load_body_biomechanics()
    standard_human_positions = bio.tracker_mapping.apply(_upright_rtmpose_pose())

    com_fraction = bio.de_leva["thigh"].com_fraction
    hip = standard_human_positions["left_hip"]
    knee = standard_human_positions["left_knee"]

    result = calculate_center_of_mass(standard_human_positions, bio)
