...
from freemocap.core.pipeline.realtime_pipeline.realtime_tasks.calibration_task.ooooold.calibration_helpers.calibration_numpy_types import ObjectPoint3D, \
...
def triangulate_point(image_point_by_camera: dict[CameraIdString, ImagePoint2D],
    ...
    A = np.zeros((number_of_cameras * 2,
                  4))  # SVD needs this to be a 2d array of shape (2n, 4), where n is the number of cameras
    ...
    u, s, vh = np.linalg.svd(A,
                             full_matrices=True)  # SVD returns the singular values in descending order, so the last row of vh is the solution (i.e. the 3D point represented in homogeneous coordinates)
    points_3d = vh[-1]
...
def calculate_reprojection_error(object_points: ObjectPoints3D,
                                 ...
                                 camera_intrinsics: dict[CameraIdString, CameraIntrinsicsEstimate],
                                 camera_transforms: dict[CameraIdString, TransformationMatrix],
                                 image_sizes: dict[CameraIdString, tuple[int, int]]) -> tuple[
    list[ReprojectionError], dict[CameraIdString, list[ReprojectionError]]]:
    ...
    for camera_id in camera_intrinsics.keys():
        projected_image_points, jacobian = cv2.projectPoints(objectPoints=object_points,
                                                             ...
                                                             aspectRatio=image_sizes[camera_id][0] /
                                                                         image_sizes[camera_id][1]
                                                             )

        reprojection_error_per_point_by_camera[camera_id] = np.abs(
            image_points_by_camera[camera_id] - np.squeeze(projected_image_points))
