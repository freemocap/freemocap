        ...
        while not multi_cam.exit_event.is_set():   
            if not multi_cam.multi_cam_tuple_queue.empty():
                ...
                for this_cam_num in range(multi_cam.num_cams):
                    ...
                    #     charuco_corners[this_cam_num], charuco_ids[this_cam_num], aruco_square_corners[this_cam_num], aruco_square_ids[this_cam_num] = detect_charuco_board(this_cam_image)
                    ...
                    # image_w_markers = cv2.aruco.drawDetectedCornersCharuco(this_cam_image, np.array(charuco_corners[this_cam_num]), np.array(charuco_ids[this_cam_num]))
                    
                    # this_cam_name = "Camera " + str(this_cam_num)
                    
                    # text_to_write_on_this_camera =''
                    # current_cam_corner_count_str    = this_cam_name + ": " + str(len(charuco_ids[this_cam_num])) + " of " + str(num_charuco_corners) + " detected | Full Board: " + str(full_charuco_detected_on_this_frame[this_cam_num])
                    # this_cam_shared_views_str = " | Shared Views: " + str(each_cameras_shared_board_view_count_total[this_cam_num])
                    ...
                    #     cv2.polylines(image_w_markers, np.int32([charuco_corners[this_cam_num]]), True, (0,100,255), 2)
                ...
                for this_pair_num, this_cam_pair in enumerate(camera_pair_list):
                    ...
                    if full_charuco_detected_on_this_frame[camera_one_id] and full_charuco_detected_on_this_frame[camera_two_id]:
