...
class SettingsManager:
    ...
    def update_from_app(self, app: "FreemocapApplication") -> None:
        ...
        All config values are read from the live application objects
        (pipeline, camera groups) so the settings blob is always
        the single source of truth. Runtime status fields (is_recording,
        recording_progress, etc.) are preserved from the internal state
        since they are only tracked here, not on the pipeline.
        """
        # Camera state — read from live camera groups
        ...
        # Pipeline state — read from live pipeline
        ...
        # Calibration — read config from the live pipeline when available,
        # fall back to internal state when no pipeline exists.
        # Runtime status fields are always from internal state.
        calibration_config = (
            first_pipeline.config.calibration_config
            if first_pipeline is not None
            else self._settings.calibration.config
        )
        ...
        # Mocap — same pattern: read config from live pipeline when available.
        mocap_config = (
            first_pipeline.config.mocap_config
            if first_pipeline is not None
            else self._settings.mocap.config
        )
        mocap = MocapSettings(
            config=mocap_config,
