...
from freemocap.system.paths_and_filenames.path_getters import (
    get_last_successful_calibration_toml_path, get_recording_session_folder_path,
)

logger = logging.getLogger(__name__)


class CalibrationPickerWidget(QWidget):
    ...
    def update_calibration_toml_path(self, toml_path: str = None):
        if toml_path is None:
            if self._load_calibration_from_file_radio_button.isChecked():
                if self._user_selected_toml_path is not None:
                    ...
                    toml_path = self._calibration_toml_path
        ...
        if self._calibration_toml_path is not None:
            ...
            if not Path(self._calibration_toml_path).exists():
    ...
    def _add_calibrate_from_active_recording_radio_button(self):
        ...
        self._calibrate_from_active_recording_radio_button.toggled.connect(lambda: NotImplementedError())
        ...
        self._calibrate_from_active_recording_button.clicked.connect(self.calibrate_from_selected_toml)
    ...
    def _create_charuco_square_size_form_layout(self):
        ...
        self._charuco_square_size_line_edit.setText("")
    ...
    def calibrate_from_selected_toml(self, charuco_square_size_mm: float = None):
        ...
        self._anipose_calibration_frame_worker = AniposeCalibrationThreadWorker(
            calibration_videos_folder_path=str(Path(self._calibration_toml_path).parent),
            charuco_square_size=float(charuco_square_size_mm),
            kill_thread_event=threading.Event(),#dummy
        )
...
if __name__ == "__main__":
    ...
    widget = CalibrationPickerWidget()
    widget.show()
    sys.exit(app.exec())
