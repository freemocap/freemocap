...
from freemocap.system.paths_and_files_names import MEDIAPIPE_IMAGE_NPY_FILE_NAME, ANNOTATED_VIDEOS_FOLDER_NAME, \
...
@dataclass
class MediapipeNumpyArrays:
    body_frameNumber_trackedPointNumber_XYZ: np.ndarray  # calling mediapipe's "pose" data "body"
    body_world_frameNumber_trackedPointNumber_XYZ: np.ndarray  # mediapipe's "pose_world_landmarks" data
    rightHand_frameNumber_trackedPointNumber_XYZ: np.ndarray
    leftHand_frameNumber_trackedPointNumber_XYZ: np.ndarray
    face_frameNumber_trackedPointNumber_XYZ: np.ndarray

    body_frameNumber_trackedPointNumber_confidence: np.ndarray
    ...
    @property
    def all_data_nFrames_nTrackedPts_XYZ(self):
        """dimensions will be [number_of_frames , number_of_markers, XYZ]"""
...
class MediaPipeSkeletonDetector:
    ...
    def process_folder_full_of_videos(
        ...
        mediapipe_single_camera_npy_arrays_list = []
        for video_number, synchronized_video_file_path in enumerate(path_to_folder_of_videos_to_process.glob("*.mp4")):
            ...
            for frame_number in tqdm(
                ...
                mediapipe_data_payload = self.detect_skeleton_in_image(raw_image=image)
                video_mediapipe_results_list.append(mediapipe_data_payload.mediapipe_results)
                annotated_image = self._annotate_image(image, mediapipe_data_payload.mediapipe_results)
            ...
            camera_mediapipe_single_camera_npy_arrays = self._list_of_mediapipe_results_to_npy_arrays(
                video_mediapipe_results_list,
                image_width=video_width,
                image_height=video_height,
            )

            mediapipe_single_camera_npy_arrays_list.append(camera_mediapipe_single_camera_npy_arrays)

        all_cameras_data2d_list = [
            m2d.all_data_nFrames_nTrackedPts_XYZ for m2d in mediapipe_single_camera_npy_arrays_list
        ]

        all_cameras_pose_world_data_list = [
            m2d.body_world_frameNumber_trackedPointNumber_XYZ for m2d in mediapipe_single_camera_npy_arrays_list
        ]

        all_cameras_right_hand_world_data_list = [
            m2d.rightHand_frameNumber_trackedPointNumber_XYZ for m2d in mediapipe_single_camera_npy_arrays_list
        ]

        all_cameras_left_hand_world_data_list = [
            m2d.leftHand_frameNumber_trackedPointNumber_XYZ for m2d in mediapipe_single_camera_npy_arrays_list
        ]

        all_cameras_face_world_data_list = [
            m2d.face_frameNumber_trackedPointNumber_XYZ for m2d in mediapipe_single_camera_npy_arrays_list
        ]
        ...
        image_data_numCams_numFrames_numTrackedPts_XYZ = np.empty(
            (
                number_of_cameras,
                number_of_frames,
        ...
        for cam_num in range(number_of_cameras):
            image_data_numCams_numFrames_numTrackedPts_XYZ[cam_num, :, :, :] = all_cameras_data2d_list[cam_num]
        ...
        self._save_mediapipe_data_to_npy(
    ...
    def _save_mediapipe_data_to_npy(self,
                                    image_data_numCams_numFrames_numTrackedPts_XYZ: np.ndarray,
                                    body_world_numCams_numFrames_numTrackedPts_XYZ: np.ndarray,
                                    output_data_folder_path: Union[str, Path],
                                    ):
        mediapipe_image_data_save_path = Path(output_data_folder_path) / MEDIAPIPE_IMAGE_NPY_FILE_NAME
        mediapipe_image_data_save_path.parent.mkdir(exist_ok=True, parents=True)
        logger.info(f"saving mediapipe image npy file: {mediapipe_image_data_save_path}")
        np.save(str(mediapipe_image_data_save_path), image_data_numCams_numFrames_numTrackedPts_XYZ)
    ...
    def _threshold_by_confidence(
            self,
            image_data_trackedPoint_dim: np.ndarray,
            image_data_trackedPoint_confidence: np.ndarray,
            confidence_threshold: float,
    ):

        threshold_mask = image_data_trackedPoint_confidence < confidence_threshold
        image_data_trackedPoint_dim[threshold_mask, :] = np.nan
        return image_data_trackedPoint_dim
