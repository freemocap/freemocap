...
- `freemocap_anipose.py` — FreeMoCap interface to aniposelib
- `run_anipose_calibration.py` — Orchestrates the full calibration pipeline
- `bundle_adjust.py` — Bundle adjustment logic
- `camera_model_solver_ops.py` — Camera model construction from solver output
...
The output of calibration is a `CalibrationResult` (Pydantic model, 220 lines):

```python
class CalibrationResult(BaseModel):
    cameras: list[CameraModel]          # One per calibrated camera
    board: CharucoBoardDefinition       # Board geometry
    reprojection_error: float           # Aggregate reprojection error (pixels)
    solver_stats: dict                  # Solver-specific statistics
```
...
Serializes to / deserializes from anipose-compatible TOML format. Provides `get_triangulator()` for building a `Triangulator` from the calibration result.

## CalibrationStateTracker (Realtime)

Used by `RealtimeAggregatorNode` for live calibration during realtime sessions (`calibration_state.py`, 368 lines):

- **Optimistic load on creation**: loads the most recent calibration TOML immediately
- **Hot-reload**: polls for file changes every 1 second — if you run a new calibration on disk, the realtime pipeline picks it up without restarting
...
Orchestrated by `posthoc_calibration_task.py` (337 lines):
