...
- The user confirms posthoc processing completes in the app; cosmetic QA is deferred. Canonical
  playback manifest/window routes and the existing 3D viewport adapter are connected. Current
  playback acceptance and remaining clock/media wiring are tracked in [app workflow TODO](app-workflow-todo.md).
...
- `saved_reconstruction.py` loads selected run/group/tracker raw point arrays, timestamps, model
  and frozen fit from Parquet under the recording lock. Explicit identity policy performs no
  filtering. Nulls, nonzero frame numbers and irregular timestamps survive reload; incomplete
  component grids fail. Input fingerprints cover points/timing/channel, model, fit and options.
  `RecordingFitInputs` captures named numeric points, model and fitting algorithm version at the
  fitting boundary. Saved-fit reconstruction rejects changed inputs before numerical work.
- Publication verifies fit evidence against the published points/model and atomically records
  FILTERING (explicit identity policy), SCALE_FIT, RECONSTRUCTION and, when every reconstructed
  model computes centre of mass, BIOMECHANICS completion. Signatures include sample timing,
  spatial channel definitions, fitted values and computation options. Timing/observation/
  triangulation completion and reader-to-planner signature resolution are not implemented yet.
  Existing descriptors without required fit-input evidence must be regenerated by processing.
- Validation: **91 focused tests pass**, including human numerical replay from saved points, model and fit,
...
1. Resolve expected group-stage signatures from saved inputs for planner validation. Numerical
   completion publication and strict fit-input binding are implemented; automatic worker reuse is not.
   Expose point-only loading for scale-fit restarts so a stale saved fit does not block refitting.
   Define filtered-input persistence and fitting eligibility before introducing non-identity filtering.
...
The disk contract, planner and raw-input reconstruction reload have focused validation. The next
deliverable is saved-input signature resolution for the planner, followed by saved-data worker execution. See
