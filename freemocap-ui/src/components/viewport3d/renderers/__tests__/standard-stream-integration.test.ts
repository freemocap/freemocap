...
function testInstancesPlacedFromWire(): void {
    ...
    for (const name of origins.names) {
        ...
        const restOrientation = schema.rest_pose!.orientations[name];
        const matrix = computeBoneMatrix(origin, quat, restOrientation, schema.segment_axes[name], length, 20.0);
        if (matrix === null) {
...
function testWireQuaternionDrivesBoneAxes(): void {
    ...
    const matrix = computeBoneMatrix(origin, quat, [1, 0, 0, 0], "z", 100.0, 1.0);
