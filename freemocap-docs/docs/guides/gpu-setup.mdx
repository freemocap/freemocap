...
FreeMoCap uses ONNX Runtime for GPU-accelerated pose detection via RTMPose. GPU acceleration significantly improves realtime pipeline throughput — especially with multiple cameras. The models and execution-provider selection are implemented in [SkellyTracker](https://freemocap.github.io/skellytracker/guides/gpu-setup); FreeMoCap configures them through the pipeline.
