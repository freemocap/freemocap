...
| # | Defect | Fix | Pass |
|---|---|---|---|
| D1 | `orientation_solver.py` composed parent-relative rotation as `world * conj(parent)`; the convention requires `conj(parent) * world`. `ROTATIONS_LOCAL` was wrong for any segment whose parent is rotated — and that is what VMC consumes. | **✅ FIXED 2026-08-12.** Confirmed empirically first (4/4 assertions red), then corrected to `parent_world.conjugate() * world`; **22/22 green**. The `FrameOrientationResult` docstring, which stated the reversed form, was corrected with it. Swept the rest of the engine for the same pattern — the only other `conj` composition is `quaternion_math.py`'s angular-velocity finite difference (`q_next * conj(q_curr)`), which is the **spatial** angular velocity of a single body between frames, not a hierarchy composition, and is correct. | ✅ DONE |
