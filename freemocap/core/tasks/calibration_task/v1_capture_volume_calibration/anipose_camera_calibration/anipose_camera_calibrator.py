...
from freemocap.core.tasks.calibration_task.v1_capture_volume_calibration.anipose_camera_calibration import \
    freemocap_anipose
...
class AniposeCameraCalibrator:
    def __init__(
            ...
            calibration_recording_folder: Union[str, Path],
        ...
        self._recording_folder_path = Path(calibration_recording_folder)

        self._calibration_videos_folder_path = self._recording_folder_path / "synchronized_videos"
        self._get_video_paths()
        self._initialize_anipose_objects()

    @property
    def calibration_toml_path(self) -> Path:
        recording_name = self._recording_folder_path.stem
        toml_name = f"{recording_name}_camera_calibration.toml"
        return self._recording_folder_path / toml_name

    def _get_video_paths(self):
        self._list_of_video_paths = get_video_paths(path_to_video_folder=Path(self._calibration_videos_folder_path))
        if len(self._list_of_video_paths) == 0:
            error_message = f"No calibration videos found in folder: {str(self._calibration_videos_folder_path)}"
            logger.error(error_message)
            raise ValueError(error_message)
        if len(self._list_of_video_paths) < 2:
            error_message = f"At least two calibration videos are required for camera calibration, but only {len(self._list_of_video_paths)} video(s) were found in folder: {str(self._calibration_videos_folder_path)}"
            logger.error(error_message)
            raise ValueError(error_message)
    ...
    def calibrate_camera_capture_volume(self,
        ...
        self._anipose_camera_group_object.dump(self.calibration_toml_path)
        logger.debug(
            f"anipose camera calibration data saved tor - {str(self.calibration_toml_path)}"
        )
