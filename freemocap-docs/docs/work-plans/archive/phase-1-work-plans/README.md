...
> Parent spec: [09](../streaming-compatibility-specs/09-standard-stream-protocol.md), [06](../streaming-compatibility-specs/06-backend-refactor-and-cleanup.md),
> [05](../streaming-compatibility-specs/05-ui-integration-and-refactor.md), [01](../streaming-compatibility-specs/01-canonical-data-model.md).
...
- Rotations are either declared-NaN or populated live — see the [sequencing decision](#key-sequencing-decision--superseded-2026-08-11-recorded-here-2026-08-13).

## Workstreams

| WS | Scope | Realizes | Depends on |
|---|---|---|---|
| **FMC-WS-1 — Standard-stream contract** | schema + sample **types + wire codecs**, no wiring yet | [09](../streaming-compatibility-specs/09-standard-stream-protocol.md) | — (linchpin) |
| **FMC-WS-2 — Backend encoder + WS reshape** | produce schema/samples from the canonical frame; reshape `websocket_server.py` send path | [06](../streaming-compatibility-specs/06-backend-refactor-and-cleanup.md) | FMC-WS-1, FMC-WS-3 |
| **FMC-WS-3 — Canonical-frame extensions** | convention-in-schema, subject dimension, confidence/error, rotation-channel declaration | [01](../streaming-compatibility-specs/01-canonical-data-model.md) | FMC-WS-1 |
| **FMC-WS-4 — UI wedge + consumption** | extract connection/transport service; decode schema+samples → 3D viewport | [05](../streaming-compatibility-specs/05-ui-integration-and-refactor.md) | FMC-WS-1 |
| **FMC-WS-5 — Kinematics engine fold-in → rotations** | copy/adapt `bs/kinematics_core` into SkellyForge; standard-human rig; live per-segment quaternions → fill rotation channels | [11](../streaming-compatibility-specs/11-kinematics-fold-in.md), [12](../streaming-compatibility-specs/12-standard-human-model.md), [13](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md) | FMC-WS-1 (channel) |
| **FMC-WS-6 — Tests & verification** | golden bytes, round-trip, LSL-model parity, UI decode parity | [08](../streaming-compatibility-specs/08-testing-strategy.md) | threads all |
...
and the current handoff is `HANDOFF_2026-08-13.md` (unavailable historical reference: `HANDOFF_2026-08-13.md`). (FMC-WS-5 was absorbed
