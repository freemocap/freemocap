...
- **Shutdown signal**: Each node holds a `shutdown_self_flag` (`multiprocessing.Event`) checked in its main loop
...
The `should_continue` property checks all three stop conditions: `not shutdown_flag.is_set() and not global_kill_flag.value and ws_queue is not None`.
...
3. **Runs detectors inline**: `RTMPoseDetector` for skeleton, `CharucoDetector` for calibration markers
...
### RealtimeSkeletonInferenceNode (`realtime_skeleton_inference_node.py`, 430 lines)
...
7. **TensorRT engine compilation** on first run (1–3 minutes, cached on subsequent runs)
...
1. **Owns a `CalibrationStateTracker`** for hot-reloadable calibration (loads on creation, polls for changes every 1s)
...
5. **Triangulation**: DLT via `Triangulator`, reprojection error gating
...
### VideoGroupHelper (`video_group_helper.py`, 555 lines)
...
class PipelineProgressMessage:
    pipeline_id: str
    pipeline_type: PosthocPipelineType  # CALIBRATION or MOCAP
