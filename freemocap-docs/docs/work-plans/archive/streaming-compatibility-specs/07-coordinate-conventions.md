...
| `rotation_frame` | local, world | Are segment rotations relative to the parent segment (local) or to the world? **Under revision** — the stream now ships *both* frames as distinct channel groups, so a single value can no longer describe it. See [FMC-SR §2](../phase-1-work-plans/07-spec-reconciliation.md#2-channel-groups--resolve-the-09-vs-03-conflict). |
...
> invariant. Tracked as **D35** in [FMC-SR](../phase-1-work-plans/07-spec-reconciliation.md#10-defect-register--everything-found-nothing-deferred).
