...
graph TD
    ...
    subgraph Track["skellytracker/"]
        Tracker["Pose Estimation<br/>unified Tracker API<br/>+ batched inference"]
    end
...
<Tip shortInfo="SkellyTracker has its own documentation at freemocap.github.io/skellytracker. It owns pose estimation — the unified Tracker → Session → Detector API, the detectors (MediaPipe, RTMPose, YOLOX, ArUco, ChArUco), and batched multi-camera inference. Freemocap imports it as a library; see the Tracking Integration page." />
...
- [Tracking Integration](/docs/architecture/tracking-integration) — How FreeMoCap builds and drives SkellyTracker (batched inference, tracker schemas)
...
- **SkellyTracker**: [freemocap.github.io/skellytracker](https://freemocap.github.io/skellytracker/) — the unified Tracker → Session → Detector API, detectors, sessions, and batched multi-camera inference. Integrated as a library dependency; see [Tracking Integration](/docs/architecture/tracking-integration).
