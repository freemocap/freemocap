...
> dual-numbered spec set (`00–14` + `phase-1/`) is preserved verbatim under [`archive/`](../index.md) — it
...
| # | Layer | Covers |
|---|-------|--------|
| **00** | [foundation/](00-foundation/index.md) | Conventions (frames, units, quaternions), the keypoint/segment vocabulary, testing philosophy — the facts every other layer assumes. |
| **01** | [data-model/](01-data-model/index.md) | The standard-human structures: the VRM segment model, T-pose reference geometry, tracker→standard-human mappings, and the message contract (envelope, kinds, self-describing channel block). |
| **02** | [pipeline/](02-pipeline/index.md) | The engine: kinematics (orientation solver, two-tier twist), segment-length estimation + fitting, the realtime loop, the posthoc path. |
| **03** | [transport/](03-transport/index.md) | The wire: the message protocol, the backend relay + WebSocket send-path, the streaming hub + LSL/VMC adapters, the HTTP control plane, on-disk serialization. |
| **04** | [ui/](04-ui/index.md) | The frontend: the message dispatcher (TransportService), the client homes, the rigid-body renderers. |
