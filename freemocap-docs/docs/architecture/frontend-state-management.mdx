...
State management uses **Redux Toolkit** with 12 slice reducers, two custom listener middlewares, and selective localStorage persistence. Rapidly-changing data (camera frames, keypoints) bypasses Redux entirely and lives in refs inside `ServerContextProvider`.
...
### videos
**State**: `folder`, `files[]`, `selectedFile`, `playbackState`.
...
### realtime
**State**: `pipelineConfig` (camera node config, aggregator config), `cameraGroupId`, `pipelineId`, `isConnected`, `isLoading`.
...
### calibration
**State**: `config` (charuco board squares/rows/length, solver method, min shared views), `isRecording`, `recordingProgress`, `directoryInfo`, `loadedCalibration`.
...
### mocap
**State**: `config` (MediaPipe detector settings: model complexity, confidence thresholds, segmentation, face refinements; skeleton filter settings: One Euro Filter, FABRIK, bone length estimation, point gating, prediction), `isRecording`, `recordingProgress`, `processingProgress`, `processingPhase`, `directoryInfo`, `calibrationTomlPath`.
...
### locale
**State**: Current language code, previous language code.
...
### blender
**State**: `blenderExePath`, `detectedBlenderExePath`, `exportToBlenderEnabled`, `autoOpenBlendFile`, `isExporting`, `isDetecting`, `isOpening`, `lastBlendFilePath`.
...
### recordingStatus
**State**: `byRecordingId` (Record of per-recording status entries), `recordingsList[]`, `recordingsIsLoading`.
...
Every slice follows the same file structure. This is a strict convention — new slices must follow it:
