...
|---|---|
| Camera Config Tree | Streaming |
| Calibration Module | Active Recording, Streaming |
| Recording Path | All tabs |
| Recording Control | All tabs |
| Process Mocap | Active Recording |
| Mocap Panel | Active Recording |
...
- **Streaming tab**: Framerate viewer (D3 charts) on the left, log terminal on the right
- **Playback / Active Recording tabs**: Log terminal fills the space
...
│   ├── control-panels/           Sidebar panels (camera-config, mocap, recording, realtime, server-connection)
│   ├── framerate-viewer/         D3 framerate charts (timeseries, histogram, statistics)
│   ├── playback/                 PlaybackContext, SyncedVideoPlayer, RecordingBrowser, PlaybackControls
│   ├── pipeline-progress/        PipelineProgressPanel, PipelineProgressBar, PipelineProgressSnackbar
...
│   └── common/                   ErrorBoundary, DirectoryStatusPanel, PresetPicker, CollapsibleSidebarSection
│
...
│   │   └── server-helpers/           canvas-manager, frame-processor, image-overlay, log-store, websocket-connection
│   └── electron-ipc/                  tRPC proxy over Electron IPC
│
├── store/             Redux state management
│   ├── store.ts                 configureStore (12 slices, 2 middleware)
...
│   └── slices/                  One directory per slice
│
...
├── i18n/              Internationalization (i18next, 40 locales)
├── types/             Shared TypeScript types
├── utils/             Pure utility functions
├── constants/         URLs, external links
└── assets/            SVG icons (~80 icons)
```
