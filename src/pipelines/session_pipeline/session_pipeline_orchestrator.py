...
from src.config.home_dir import create_session_folder, create_session_id, \
    get_most_recent_session_id, get_session_output_data_folder_path
...
class SessionPipelineOrchestrator:
    ...
    def calibrate_camera_capture_volume(self, use_most_recent_calibration: bool = False,
        ...
        if use_most_recent_calibration:
            self._anipose_camera_calibration_object = CalibrationPipelineOrchestrator(

            ).load_most_recent_calibration()
            return
    ...
    def record_new_session(
        ...
        if reconstruct_3d:
            try:
                self.anipose_camera_calibration_object = CalibrationPipelineOrchestrator(
        ...
        with self._open_cv_camera_manager.start_capture_session_all_cams() as \
                        ...
                        if detect_charuco:
                            if len(dictionary_of_charuco_frame_payloads_on_this_multi_frame) > 0:
                                charuco3d_multi_frame_payload = self._reconstruct_3d_charuco(
                                    dictionary_of_charuco_frame_payloads_on_this_multi_frame)
                                if show_visualizer_gui and charuco3d_multi_frame_payload is not \
                                    None:
                        ...
                        if detect_mediapipe:
                            if len(dictionary_of_mediapipe_payloads_on_this_multi_frame) > 0:
                                mediapipe3d_multi_frame_payload = self._reconstruct_3d_mediapipe(
                                    dictionary_of_mediapipe_payloads_on_this_multi_frame)
                                if show_visualizer_gui and mediapipe3d_multi_frame_payload is not\
                                    None:
                    ...
                    for this_webcam_id, this_open_cv_camera in connected_cameras_dict.items():
                        ...
                        if detect_mediapipe:
                            ...
                            this_mediapipe_frame_payload = \
                                self._mediapipe_skeleton_detector.detect_skeleton_in_image(
                            ...
                            if reconstruct_3d:
                                dictionary_of_mediapipe_payloads_on_this_multi_frame[
                                    this_webcam_id] = this_mediapipe_frame_payload
    ...
    def _reconstruct_3d_charuco(self, dictionary_of_charuco_payloads_on_this_multiframe) -> \
        ...
        if sum(
            is_there_any_charuco_data) >= min_cameras_to_reconstruct:  # if at least 2 cameras
    ...
    def _format_charuco2d_data(self, dictionary_of_charuco_payloads_on_this_multiframe) -> Dict:
        ...
        for this_cam_data in dictionary_of_charuco_payloads_on_this_multiframe.values():
            ...
            if charuco_ids_in_this_frame_idx.__class__ != list:
                this_frame_charuco_data_xy[charuco_ids_in_this_frame_idx,
    ...
    def _reconstruct_3d_mediapipe(self, dictionary_of_mediapipe_payloads_on_this_multi_frame) -> \
        ...
        min_cameras_to_reconstruct = 2
        if sum(
            is_there_any_mediapipe_data) >= min_cameras_to_reconstruct:  # if at least 2 cameras
            # have any data, then we can try to make some 3d dottos
            mediapipe2d_data_per_cam_dict = {}
            for this_webcam_id, this_mediapipe_payload in \
                dictionary_of_mediapipe_payloads_on_this_multi_frame.items():
                self._threshold_2d_data_by_confidence(
                    this_mediapipe_payload.pixel_data_numpy_arrays, .5)
    ...
    def reconstruct3d_from_2d_data_offline(self):
        ...
        data3d_reprojectionError_flat = self.anipose_camera_calibration_object.reprojection_error(
            data3d_flat,
            data2d_flat,
            mean=True)

        data3d_numFrames_numTrackedPoints_XYZ = data3d_flat.reshape(number_of_frames,
                                                                    number_of_tracked_points, 3)
        data3d_numFrames_numTrackedPoints_reprojectionError = data3d_reprojectionError_flat.reshape(
            number_of_frames,
        ...
        self._mediapipe3d_full_session_payload = Data3dFullSessionPayload(
            ...
            data3d_numFrames_numTrackedPoint_reprojectionError
            =data3d_numFrames_numTrackedPoints_reprojectionError)
    ...
    def _save_mediapipe3d_data_to_npy(self, data3d_numFrames_numTrackedPoints_XYZ: np.ndarray,
        ...
        self._mediapipe_3dData_save_path = output_data_folder / \
        ...
        self._mediapipe_reprojection_error_save_path = output_data_folder / \
        ...
        np.save(str(self._mediapipe_reprojection_error_save_path),
                data3d_numFrames_numTrackedPoints_reprojectionError)

    def _threshold_2d_data_by_confidence(self, mediapipe2d_data_payload: Mediapipe2dDataPayload,
...
def load_mediapipe3d_skeleton_data(session_id: str = None):
    ...
    mediapipe3d_xyz_file_path = output_data_folder / \
                                "mediapipe_3dData_numFrames_numTrackedPoints_spatialXYZ.npy"
    ...
    mediapipe_reprojection_error_file_path = output_data_folder / \
                                             "mediapipe_3dData_numFrames_numTrackedPoints_reprojectionError.npy"
...
def load_mediapipe2d_data(session_id: str = None):
    if session_id is None:
        session_id = get_most_recent_session_id()
    output_data_folder = Path(get_session_output_data_folder_path(session_id))
    mediapipe2d_xy_file_path = output_data_folder / \
