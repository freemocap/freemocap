...
| Workflow | Purpose |
|---|---|
| `build-installers.yml` | Builds platform-specific installers on push to main |
| `deploy-docs.yml` | Builds and deploys this documentation site to GitHub Pages |
| `test.yml` | Runs backend + frontend tests on PR |

## Versioning

- **Backend version**: `v1.4.7` (in `freemocap/__init__.py`), managed by `bumpver`
...
<Tip shortInfo="The version numbers are currently out of sync between the Python backend (v1.4.7) and the frontend (v2.0.0-alpha.1). This reflects the v2 rewrite of the frontend while the backend is being incrementally updated." />
