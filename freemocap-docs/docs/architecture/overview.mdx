...
graph TD
    ...
    subgraph Python["Python Backend<br/>(freemocap/)"]
        FastAPI["FastAPI REST API<br/>:53117"]
...
New code targets the current architecture without shims or migration layers. However, the project retains some legacy compatibility: the `RecordingStructure` model supports both canonical and legacy recording layouts, the old MUI-based UI still exists in `freemocap-ui-old/` as a reference, and some pipeline configs offer legacy operating modes (e.g., inline vs centralized GPU inference). Legacy support is pragmatic — it keeps existing recordings usable while the system evolves.
