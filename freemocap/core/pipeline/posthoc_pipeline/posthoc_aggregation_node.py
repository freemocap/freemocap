...
class PosthocAggregationNode:
    ...
    def create(cls,
        ...
        worker = multiprocessing.Process(target=cls._run,
                                         name=f"CameraGroup-{camera_group_id}-PosthocAggregationNode",
    ...
    def _run(config: PipelineConfig,
        ...
        logger.debug("PosthocAggregationNode process started - waiting for SHM message")
        shm_message: SetShmMessage = shm_subscription.get(block=True)
        logger.debug("PosthocAggregationNode - received SHM message - starting main loop")
        try:
            ...
            while ipc.should_continue and not shutdown_self_flag.value:
                ...
                while not pipeline_config_subscription.empty():
                    ...
                    logger.info(f"PosthocAggregationNode for camera group {camera_group_id} received updated config")
            ...
            logger.error(f"Exception in PosthocAggregationNode for camera group {camera_group_id}: {e}", exc_info=True)
            ipc.kill_everything()
            raise
        finally:
