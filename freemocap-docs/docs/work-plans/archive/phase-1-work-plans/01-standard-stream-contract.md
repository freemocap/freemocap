...
> (that is FMC-WS-2). Realizes [09 — Standard Stream Protocol](../streaming-compatibility-specs/09-standard-stream-protocol.md).
...
- `coordinate_convention.py` — the `CoordinateConvention` value type ([07](../streaming-compatibility-specs/07-coordinate-conventions.md)).
...
share these types for the disk sidecar / tidy format ([10](../streaming-compatibility-specs/10-serialization-and-tidy-format.md)) if it makes
...
([13](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md), [12](../streaming-compatibility-specs/12-standard-human-model.md)).
...
1. [x] `CoordinateConvention` type + canonical default — `coordinate_convention.py`.
   **Corrected 2026-08-12:** the forward-axis was never a research question, it was an undeclared standard.
   ...
   ([07](../streaming-compatibility-specs/07-coordinate-conventions.md#the-freemocap-canonical-convention)). The code's `+Y` value and its
...
6. [x] Module docstrings as the SSOT for the wire contract (link [09](../streaming-compatibility-specs/09-standard-stream-protocol.md)).
