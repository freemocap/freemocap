...
> [11](../streaming-compatibility-specs/11-kinematics-fold-in.md), [12](../streaming-compatibility-specs/12-standard-human-model.md), [13](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md).
...
- **5b — Standard-human model.** VRM rig (bones), landmark→bone retarget, T-pose rest pose, and the
  **`anatomical_offset`** mapping form (SC / GH / hip centers) in the skellytracker mappings + canonical model
  ([13](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md)).
- **5c — Twist policy.** Per-bone axis-source policy (full-frame → chain/hinge-resolved → damped-minimal
  fallback) — [12](../streaming-compatibility-specs/12-standard-human-model.md).
