...
> is the current phase. Scope + progress: `IMPLEMENTATION_PLAN.md` (unavailable historical reference: `IMPLEMENTATION_PLAN.md`); the
> current-path plan: [`phase-1/10`](../phase-1-work-plans/10-whole-project-alignment.md); **start at**
> `phase-1/HANDOFF_2026-08-13.md` (unavailable historical reference: `phase-1/HANDOFF_2026-08-13.md`). Note: the *vocabulary* reframed
> in 2026-08 — "canonical keypoints" and "landmarks" are retired; the boundary is keypoint → segment
> reference geometry (see [phase-1/09](../phase-1-work-plans/09-segment-model.md)); several of the numbered docs
...
| — | Implementation Plan (unavailable historical reference: `IMPLEMENTATION_PLAN.md`) | Scope table, phased build order, dependencies, progress log |
...
- **Scope tags** — every major feature is tagged **`[IN]`** (near-term), **`[LATER]`**, or
  **`[FUTURE]`** so aspiration is never confused with commitment. The authoritative scope table lives
  in `IMPLEMENTATION_PLAN.md` (unavailable historical reference: `IMPLEMENTATION_PLAN.md`).
...
- **Audits** — `AUDIT_<YYYY-MM-DD>.md`, one per checkpoint audit of plans-vs-implementation. An audit
  ...
  owns it (`IMPLEMENTATION_PLAN.md` (unavailable historical reference: `IMPLEMENTATION_PLAN.md`) § Dependencies & blockers for open questions,
