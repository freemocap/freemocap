...
async def cv2_imshow_one_camera(webcam_id: str, camera_preview_model: CameraPreviewModel = CameraPreviewModel()):
    ...
    with cv_cam_manager.start_capture_session_all_cams() as connected_cameras_dict:
        ...
        while should_continue:
            ...
            if exit_key == 27:
                logger.info("ESC has been pressed.")
                should_continue = False
                cv2.destroyAllWindows() #TODO - close each named window specificially by name
