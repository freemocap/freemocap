...
class PointTriangulator:
    ...
    def create(cls):
        ...
        camera_calibrations = {CameraId(camera_id): CameraCalibrationData.from_tuple(data) for camera_id, data in
                               enumerate(calibration_data.items()) if camera_id != "metadata"}
...
def _validate_input(points2d_by_camera: np.ndarray, camera_extrinsic_matricies: np.ndarray):
    ...
    if number_of_cameras != camera_extrinsic_matricies.shape[0]:
        raise ValueError(
            f"Expected number of cameras to match, got {number_of_cameras} and {camera_extrinsic_matricies.shape[0]}")
    if camera_extrinsic_matricies.shape[1:] != (3, 4):
        raise ValueError(
            f"Expected camera extrinsic matricies to be of shape (num_cams, 3, 4), got {camera_extrinsic_matricies.shape}")
...
if __name__ == "__main__":
    ...
    _camera_extrinsic_matricies = np.array(
