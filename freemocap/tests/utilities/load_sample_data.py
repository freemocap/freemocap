...
from freemocap.system.paths_and_files_names import (
    ...
    FIGSHARE_TEST_DATA_FILE_NAME,
    FIGSHARE_TEST_DATA_ZIP_FILE_URL,
...
def load_sample_data(sample_data_zip_file_url: str = FIGSHARE_TEST_DATA_ZIP_FILE_URL) -> str:
    try:
        ...
        r = requests.get(FIGSHARE_TEST_DATA_ZIP_FILE_URL, stream=True)
        r.raise_for_status()  # Check if request was successful

        z = zipfile.ZipFile(io.BytesIO(r.content))
        z.extractall(recording_session_folder_path)

        figshare_sample_data_path = recording_session_folder_path / FIGSHARE_TEST_DATA_FILE_NAME
