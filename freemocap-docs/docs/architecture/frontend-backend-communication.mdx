...
`FreemocapApplication` holds the camera-group, realtime-pipeline, and posthoc-pipeline
managers. These server-owned objects outlive individual client views and connections.
...
| `freemocap/app/freemocap_application.py` | Manager fields, `create`, `create_or_update_realtime_pipeline` |
