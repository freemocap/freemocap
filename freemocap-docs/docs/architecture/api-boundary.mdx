...
```
Note: message type is `"tracker_schemas"` (plural). The `schemas` field is a dict of tracker name → `TrackerDefinition` — each with `name`, `tracked_points`, and name-pair `connections` (from `core/tracking/tracker_definitions.py`, e.g. `RTMPOSE_WHOLEBODY_DEFINITION`). Multiple schemas are sent in one message. See [Tracking Integration](/docs/architecture/tracking-integration).
