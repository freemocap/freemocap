...
| **Segment** | A 3D-oriented rigid body of the fitted model: a **transform origin** (its proximal joint), a reference geometry, an orientation. VRM-1.0-aligned — **not** an anatomical bone ([13](13-tracker-to-canonical-mapping.md#segments-not-anatomical-bones-current-scope)). Owned by **SkellyForge**. This is what the stream carries alongside the keypoints. |
