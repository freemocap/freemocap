...
> [D1](../phase-1-work-plans/07-spec-reconciliation.md#10a-correctness) empirically rather than by reading docstrings, and
...
- **Handedness survives mirroring.** After composition, assert `det(basis) == +1` for **every** segment on
  **both** sides. Mirroring reflects rest positions (negating Y, the sagittal normal) and then *rebuilds*
  ...
  [SF-AL A3](../phase-1-work-plans/08-skellyforge-alignment.md#a3--mirroring-reflect-positions-rebuild-frames).
