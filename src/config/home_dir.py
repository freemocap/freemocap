...
def get_most_recent_session_id()->str:
    session_id_toml_path = Path(get_freemocap_data_folder_path()) / MOST_RECENT_SESSION_ID_FILENAME
...
def get_mediapipe_annotated_videos_folder_path(session_id: str, create_folder: bool = True):
...
def get_session_calibration_file_path(session_id:str)->str:
    ...
    calibration_file_path = Path(get_session_folder_path(session_id))/calibration_file_name
