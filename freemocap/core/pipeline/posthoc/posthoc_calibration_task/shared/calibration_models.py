...
from freemocap.core.pipeline.posthoc.posthoc_calibration_task.shared.toml_mixin import (
    TomlMixin,
    numpy_to_python,
)
...
class CharucoBoardDefinition(BaseModel, TomlMixin):
    ...
    def validate_geometry(self) -> "CharucoBoardDefinition":
        if self.marker_length_mm >= self.square_length_mm:
            raise ValueError(
                f"marker_length_mm ({self.marker_length_mm}) must be < "
...
class CalibrationResult(BaseModel, TomlMixin):
    ...
    serialized to/from the anipose-compatible TOML format that the
    rest of the freemocap pipeline expects.
    """
    ...
    # ---- Anipose-compatible TOML format (for downstream pipeline interop) ----
    ...
    def dump_anipose_toml(
        ...
        """Write anipose-compatible TOML that the rest of the pipeline can load.
        ...
        Per-camera format:
            ...
            size = [width, height]
            matrix = [[3x3]]
            distortions = [k1, k2, p1, p2, 0]
            rotation = [rx, ry, rz]  (Rodrigues)
            translation = [tx, ty, tz]
            world_orientation = [[3x3]]
    ...
    def load_anipose_toml(cls, path: Path) -> "CalibrationResult":
        ...
        Reads the per-camera format written by dump_anipose_toml (and also
        by the legacy AniposeCameraGroup.dump and save_calibration_toml).
        """
        path = Path(path)
        if not path.is_file():
        ...
        for key in sorted(toml_data.keys()):
            ...
            if K.shape != (3, 3):
                raise ValueError(
                    f"Camera '{key}': matrix shape {K.shape}, expected (3, 3)"
                )
        ...
        # Extract board info from metadata (may not be present in legacy files)
        ...
        board = CharucoBoardDefinition(
            ...
            marker_length_mm=board_meta.get("marker_length_mm", 0.8),
