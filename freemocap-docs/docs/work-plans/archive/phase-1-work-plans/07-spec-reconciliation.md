...
2. The vocabulary drifted. "Keypoint", "landmark", "bone", and "segment" are used inconsistently across
   ...
   [`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md) on what the channel groups enumerate.
...
data shape are right. [`00`](../streaming-compatibility-specs/00-overview.md) implies this; it should say it.

### Tasks

1. [x] **[`00`](../streaming-compatibility-specs/00-overview.md) § Glossary** — split into "the four load-bearing terms" (keypoint
...
2. [x] **[`13`](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md)** — retitled *Keypoint → Landmark Mapping*; declared
...
3. [x] **[`12`](../streaming-compatibility-specs/12-standard-human-model.md)** — added § *Segments, not anatomical bones* as the first
       section, qualifying locked decision 3.
4. [x] **[`00`](../streaming-compatibility-specs/00-overview.md) § The goal** — added § *Backbone first, endpoints later*.
...
[`01`](../streaming-compatibility-specs/01-canonical-data-model.md)'s superset principle, and settles the audit's
...
1. [x] **[`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md) § channels** — rewritten against the six-group table and
...
3. [x] **[`01`](../streaming-compatibility-specs/01-canonical-data-model.md) § what the frame carries** — table rewritten with a
...
4. [x] **[`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md)** — `segment_parents` declared as a schema field; with
...
5. [x] **[`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md) § open questions** — stale subject-count bullet deleted;
       resolved items moved into the body with a pointer left behind. Only `nominal_srate` remains open,
       now with a trigger.
6. [x] **[`07`](../streaming-compatibility-specs/07-coordinate-conventions.md)** — `rotation_frame` dropped from the convention tuple in
...
1. [x] **[`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md)** — **decided: one `OVERLAY_2D` kind with an
...
2. [x] **[`01`](../streaming-compatibility-specs/01-canonical-data-model.md)** — reprojected landmarks added to the frame table as an
       `OVERLAY_2D` row, with the residual-vs-fit rationale.
3. [x] **[`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md)** — records that reprojection reads the existing
       calibration, and that a calibration change rebuilds the stream with a new schema exactly as it
       invalidates triangulation.
4. [ ] **[`05`](../streaming-compatibility-specs/05-ui-integration-and-refactor.md)** — the UI needs a per-layer visibility toggle;
       note it.
...
6. [ ] **`IMPLEMENTATION_PLAN` (unavailable historical reference: `../IMPLEMENTATION_PLAN.md`)** — reconcile the two ID namespaces
...
[`08`](../streaming-compatibility-specs/08-testing-strategy.md) covers the wire and nothing owns the math
...
1. [x] Wrote [`14-engine-testing-strategy.md`](../streaming-compatibility-specs/14-engine-testing-strategy.md), covering §1–7 plus the
...
2. [x] Cross-linked: [`08`](../streaming-compatibility-specs/08-testing-strategy.md) retitled "(the wire)" with an explicit scope boundary,
       [`11`](../streaming-compatibility-specs/11-kinematics-fold-in.md) points at 14 and its status corrected to "executed",
       `README` (unavailable historical reference: `../README.md`) table gains row 14 and marks 08 as the wire's doc.
3. [x] Suite added to `IMPLEMENTATION_PLAN` (unavailable historical reference: `../IMPLEMENTATION_PLAN.md`) `[IN]` scope, with the ordering
...
**Decision: implement real critical damping.** [`12`](../streaming-compatibility-specs/12-standard-human-model.md) and `TwistPolicy`'s
...
1. [x] **[`12`](../streaming-compatibility-specs/12-standard-human-model.md) § Critical damping — specification** added: second-order,
...
3. [x] **`damping_factor` → time constant in seconds.** The load-bearing part: a per-frame blend factor
       ...
       another. Framerate independence is asserted by [14 § critical damping](../streaming-compatibility-specs/14-engine-testing-strategy.md#5-critical-damping).
...
1. [ ] **[`07`](../streaming-compatibility-specs/07-coordinate-conventions.md)** — state the convention explicitly: does `q_world` map
       segment-frame → world, or world → segment frame? Write the parent-relative composition formula that
       follows from it.
2. [ ] Record it in `IMPLEMENTATION_PLAN` (unavailable historical reference: `../IMPLEMENTATION_PLAN.md`) § Dependencies & blockers as
...
1. [ ] `README` (unavailable historical reference: `../README.md`) status line — "Nothing here is implemented yet" is false.
2. [ ] Status headers on [`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md), [`11`](../streaming-compatibility-specs/11-kinematics-fold-in.md),
       [`12`](../streaming-compatibility-specs/12-standard-human-model.md).
3. [ ] [`06`](../streaming-compatibility-specs/06-backend-refactor-and-cleanup.md) — three dead `core/kinematics/` paths.
4. [ ] `HANDOFF` (unavailable historical reference: `HANDOFF.md`) — file map (`stream_schema_builder.py` retired), Uncommitted list,
...
[`08`](../streaming-compatibility-specs/08-testing-strategy.md) warns about ("a receiver you wrote yourself shares your
...
| # | Defect | Fix | Pass |
|---|---|---|---|
| D1 | `orientation_solver.py` composed parent-relative rotation as `world * conj(parent)`; the convention requires `conj(parent) * world`. `ROTATIONS_LOCAL` was wrong for any segment whose parent is rotated — and that is what VMC consumes. | **✅ FIXED 2026-08-12.** Confirmed empirically first (4/4 assertions red), then corrected to `parent_world.conjugate() * world`; **22/22 green**. The `FrameOrientationResult` docstring, which stated the reversed form, was corrected with it. Swept the rest of the engine for the same pattern — the only other `conj` composition is `rotation_quaternion.py`'s angular-velocity finite difference (`q_next * conj(q_curr)`), which is the **spatial** angular velocity of a single body between frames, not a hierarchy composition, and is correct. | ✅ DONE |
| D2 | The SF-SH-4 test that "passed" used a **uniform** bend, where `q_child == q_parent` and both operand orders collapse to identity. It cannot detect D1. | **✅ Addressed.** `test_uniform_bend_is_order_blind` now asserts the blindness explicitly, so nobody reintroduces a uniform-only fixture believing it covers the convention. Superseded by the differential-bend + round-trip cases. | test ✅ |
| D37 | **Order-blindness has three independent causes, not one.** Building the D1 test surfaced two beyond the known uniform-bend case: (a) a segment at its **rest orientation** has an identity world quaternion, and identity on *either* side of a pair makes it order-blind; (b) parent and child rotated about the **same axis** commute, so distinct segment *directions* are not sufficient — distinct *rotation axes* are. Three successive fixtures each had one of these holes and each passed ~half the assertions for the wrong reason. | Both are now **guard tests** in the suite (`test_tested_segments_are_not_at_rest_orientation`, `test_tested_segment_rotation_axes_are_not_parallel`) that fail if a future fixture edit reintroduces the hole. Recorded in [14 §2](../streaming-compatibility-specs/14-engine-testing-strategy.md#2-composition-convention--the-decisive-one). | test ✅ |
| D3 | `_solve_chain_resolved` passed `previous_world_quaternion=None` on both fallback paths, so damping was skipped in exactly the occlusion/singularity case it exists for. | **✅ FIXED 2026-08-12.** Both fallbacks now return `twist_resolved_by_damped_minimal=True` and the frame-level solver damps them. Guarded by `test_singularity_gate_fallback_produces_damping_state`. | ✅ DONE |
| D4 | Damping was a first-order lag (95% previous frame) documented as "critically damped". | **✅ FIXED 2026-08-12.** New `skellyforge/kinematics/critically_damped_orientation.py`: genuine second-order, damping ratio 1, per-segment angular-velocity state, solved **analytically** (not the `1/(1+x+0.48x²+0.235x³)` series game engines use to dodge `exp`). `TwistPolicy.damping_factor` → `twist_time_constant_seconds`. Runs in the tangent space (log/exp map) since SO(3) is not a vector space; `from_rotation_vector` / `to_rotation_vector` added to `RotationQuaternion`. | ✅ DONE |
| D38 | `solve_frame_orientations` needed a `timestamp_seconds` to drive a time-based filter. Giving it a default would let a caller silently disable damping by omission. | **✅ FIXED 2026-08-12.** Made required with no default — a missing argument fails loudly, silently-undamped output would not. The freemocap aggregator now passes `frame_time`, the monotonic per-frame clock it already threads to the rigidifier and CoM, so damping shares one time base with the rest of the pipeline. | ✅ DONE |
...
| D16 | Module-level mutable globals `_standard_human_cache` and `_prev_orientation_result` in the aggregator: never reset between sessions (stale damping carries across recordings) and shared if two pipelines run in one process. | **✅ FIXED 2026-08-12.** `_prev_orientation_result` deleted; the orientation history is now a `_run`-scoped local (`previous_orientation_result`), matching the existing idiom for per-run state (`prev_com`, `streaming_kinematics`). Damping state rides on `FrameOrientationResult` in skellyforge, so it is per-result by construction. `_standard_human_cache` stays: it holds an **immutable** model, so sharing it is safe — and it is superseded by **D11** anyway. | ✅ DONE |
...
| D24 | `TrackerMapping.apply()` silently omits landmarks whose source keypoints are missing, while [`00`](../streaming-compatibility-specs/00-overview.md) states "Fail loudly, no fallbacks" as a blanket principle. | Silent-skip is **correct** for per-frame occlusion — a missing landmark this frame is data, not an error. But a mapping that references a keypoint the tracker never produces **is** an error and must raise at load. Spec both halves in [`13`](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md) and carve the occlusion exception into [`00`](../streaming-compatibility-specs/00-overview.md) so the principle stays true. | SR → CODE |
...
| D26 | [`06`](../streaming-compatibility-specs/06-backend-refactor-and-cleanup.md) cites three `core/kinematics/` paths that no longer exist; `HANDOFF` says the folder was "deleted" when it was **moved**; `HANDOFF` file map lists a retired file; `HANDOFF` "Uncommitted" list is wrong; "12 tests" is now 23; [`01`](01-standard-stream-contract.md) headers name files that were renamed; `README` (unavailable historical reference: `../README.md`) says nothing is implemented. | §8, all of it. | SR |
| D27 | [`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md)'s open-questions bullet says subject count is "not in the schema", contradicting its own callout box and `max_persons` in code. | §2 task 5 — delete the stale bullet. | SR |
...
| D33 | [`01`](../streaming-compatibility-specs/01-canonical-data-model.md) stated the rest-pose contract as *"a bone reading `(0,0,0,1)` is in the rest pose"* — that is **xyzw**. This codebase's canonical order is **wxyz**, where identity is `(1,0,0,0)`. The doc asserting the identity convention had it backwards, inside the section warning about horrifying-pose bugs. | **Fixed in the §1 sweep.** Corrected to `(1, 0, 0, 0)` with the order named explicitly. Every doc that prints a literal quaternion must name its component order — added to §5's test spec as an assertion. | SR ✅ |
...
| # | Question | Answer |
|---|---|---|
| Q1 | Canonical **forward-axis** — `TBD` in the code and docs. | **✅ Resolved 2026-08-12: `+X`.** The FreeMoCap canonical convention is `mm · right-handed · +Z up · +X forward` (robotics/biomechanics standards). It is a **declared internal standard**, not something derived from whatever the calibration produced — all FreeMoCap data is in it internally, conversion happens at the adapter edge on request, and re-orientation is an explicit user action through the HTTP control plane. The `TBD` was never a research question; it was an undeclared standard. Stated in [`07`](../streaming-compatibility-specs/07-coordinate-conventions.md#the-freemocap-canonical-convention). |
| Q2 | Where the canonical segment definition lives — `human_skeleton.yaml` or Python. | **Close it in this pass** as part of D11, and record the choice in [`standard-human-model/`](standard-human-model/README.md) with its rationale. |
...
- [`09`](../streaming-compatibility-specs/09-standard-stream-protocol.md) is the sole authority on channel content; `phase-1/03` defers
  to it.
...
- The world-quaternion convention and the canonical forward-axis are both written down; no `TBD` remains in
  [`07`](../streaming-compatibility-specs/07-coordinate-conventions.md) or `coordinate_convention.py`.
...
- `IMPLEMENTATION_PLAN` (unavailable historical reference: `../IMPLEMENTATION_PLAN.md`) § Dependencies & blockers carries no entry that this
