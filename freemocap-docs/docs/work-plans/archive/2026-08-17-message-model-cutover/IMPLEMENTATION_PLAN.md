...
- ✅ **Standard-human model** — DONE 2026-08-13: the composed 60-segment human (55 VRM 1.0 bones + 5 face-detail — body + hands + face
  ...
  estimator. See [phase-1/09](../phase-1-work-plans/09-segment-model.md).
- **Kinematics engine fold-in**: copy/adapt `bs/kinematics_core` into **SkellyForge**; rewrite
  ...
  quaternions (identity == T-pose). ([11](../streaming-compatibility-specs/11-kinematics-fold-in.md)).
- **Tracker→standard-human mapping**: every keypoint the model declares produced from tracker keypoints
  ...
  (`required_keypoints()` = 76) is the interface ([13](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md), [09 Task 6](../phase-1-work-plans/09-segment-model.md)).
...
- Canonical-frame extensions: **segment-rotation channels** (world + local, via the folded-in engine),
  **subject dimension**, **convention/rest-pose in the schema**, **per-point confidence / reprojection
  error**.
- **On-disk serialization**: migrate the **parquet** schema to tidy-long ([10](../streaming-compatibility-specs/10-serialization-and-tidy-format.md)).
...
- **Align** the disabled kinematics code to the new engine/models; keep it out of hot loops until validated
  ([06](../streaming-compatibility-specs/06-backend-refactor-and-cleanup.md), [11](../streaming-compatibility-specs/11-kinematics-fold-in.md)).
- Scapula (scapulothoracic detail) via the `anatomical_offset` mechanism ([12](../streaming-compatibility-specs/12-standard-human-model.md)).
...
| ~~Landmark-vs-bone: what do `POINTS` / `OVERLAY_2D` enumerate?~~ | — | **Resolved 2026-08-13 by the reframe** — the landmark layer is retired entirely; the channel groups enumerate tracker **keypoints** (`KEYPOINTS_3D`) and **segments** (`SEGMENT_ORIGINS` + rotations), per [09-standard-stream-protocol § channels](../streaming-compatibility-specs/09-standard-stream-protocol.md#channels). FMC-WS-3 implements against it (Phase F). |
...
| ~~Does `ROTATIONS_WORLD` have a committed consumer?~~ | — | **Resolved 2026-08-12** — yes, and it stays. The stream carries **everything** (small data); [FMC-RB](../phase-1-work-plans/06-rigid-body-bone-renderer.md) drives the viewport from it directly. |
| ~~Camera parameters for 2D landmark reprojection~~ | — | **Resolved 2026-08-12** — the **existing camera calibration infrastructure**. If we reconstruct 3D, we know the cameras by definition. See [FMC-SR §3](../phase-1-work-plans/07-spec-reconciliation.md#3-2d-overlays-carry-both-detections-and-reprojections-new). |
| ~~Testing home for the SkellyForge engine~~ | — | **Resolved 2026-08-12** — new sibling doc [14 — Engine Testing Strategy](../streaming-compatibility-specs/14-engine-testing-strategy.md). [08](../streaming-compatibility-specs/08-testing-strategy.md) keeps the wire; 14 owns the math. Suite added to `[IN]` scope. |
...
Detailed workstream plans live in [`phase-1/`](../phase-1-work-plans/README.md) (FMC-WS-1…FMC-WS-5; **positions-first**, rotations
...
- [ ] Segment-rotation channel defined in the schema; the folded-in `bs/` engine produces rotations live
      (copy/adapt into SkellyForge — [11](../streaming-compatibility-specs/11-kinematics-fold-in.md)). *(not blocked — we have the code)*
...
**Current phase: F — the realtime loop** ([`phase-1/11`](../phase-1-work-plans/11-realtime-loop-completion.md):
 ...
 (Phase E, [`phase-1/12`](../phase-1-work-plans/12-posthoc-rebuild.md)) is spec'd as revisit notes and executes
...
contract)* —
[`phase-1/10`](../phase-1-work-plans/10-whole-project-alignment.md) step order; detail in
[`phase-1/09` Task 6](../phase-1-work-plans/09-segment-model.md#task-6-the-required-keypoint-contract-per-tracker).
...
- **2026-08-13 (sequencing set: realtime loop first, then posthoc; both planned)** — Per the user:
  ...
  loop proves (locked decision 8). New detail plans: [`phase-1/11`](../phase-1-work-plans/11-realtime-loop-completion.md)
  ...
  [`phase-1/12`](../phase-1-work-plans/12-posthoc-rebuild.md) (the posthoc rebuild spec as REVISIT notes: the old
...
- **2026-08-12 (whole-project alignment re-derived; D7/D8 disposed)** — The paused D7/D8 work was built
  ...
  [`phase-1/10`](../phase-1-work-plans/10-whole-project-alignment.md) re-derives the ordered whole-project path (A–G),
...
- **2026-08-12 (SF-SM aligned to keypoint → segment reference geometry)** — Per the user's notes on
  [`phase-1/09`](../phase-1-work-plans/09-segment-model.md): the "canonical keypoint" vocabulary is retired — segment
...
- **2026-08-12 (SF-AL architecture agreed)** — Worked the design through before writing code. Decisions
  recorded in [`phase-1/08`](../phase-1-work-plans/08-skellyforge-alignment.md#agreed-architecture): **compose what we
  ...
  **one human per `StandardHuman`**, multi-subject is a list; **the model layer knows nothing about
  ...
  Board inheritance**. Two guard tests added to [14 §7](../streaming-compatibility-specs/14-engine-testing-strategy.md#7-standard-human-model).
- **2026-08-12 (SF-AL — SkellyForge is not aligned to the standard human)** — Surfaced while implementing
  ...
  caught it. Plan: [`phase-1/08-skellyforge-alignment.md`](../phase-1-work-plans/08-skellyforge-alignment.md).
...
- **2026-08-12 (D3/D4 — real critical damping)** — `skellyforge/kinematics/critically_damped_orientation.py`
  implements the filter [12 § Critical damping](../streaming-compatibility-specs/12-standard-human-model.md) specifies: second-order,
...
- **2026-08-12 (D1 fixed — `ROTATIONS_LOCAL` is now correct)** — `orientation_solver.py` now composes
  ...
  [07 § Segment rotation conventions](../streaming-compatibility-specs/07-coordinate-conventions.md#segment-rotation-conventions).
  ...
  **spatial** angular velocity of one body between frames — not a hierarchy composition — and is correct.
  This is the bug doc [07 § the local-rotation trap](../streaming-compatibility-specs/07-coordinate-conventions.md#the-local-rotation-trap-vmc-and-unreal)
  warned about; it is now closed before any VMC work begins.
  **Next: D3/D4 critical damping** ([12 § Critical damping](../streaming-compatibility-specs/12-standard-human-model.md), [14 §5](../streaming-compatibility-specs/14-engine-testing-strategy.md#5-critical-damping)).
  *Uncommitted in skellyforge.*
- **2026-08-12 (engine test suite stood up; D1 confirmed)** — First code of the FMC-SR follow-up.
  ...
  [14 §2](../streaming-compatibility-specs/14-engine-testing-strategy.md#2-composition-convention--the-decisive-one):
...
- **2026-08-12 (checkpoint audit)** — Plans read against the code landed on `development-streaming`;
  ...
  **sequencing reversal was the right call**, but it was recorded only in
  [`phase-1/standard-human-model/README.md`](../phase-1-work-plans/standard-human-model/README.md) and never propagated —
  [`phase-1/README.md`](../phase-1-work-plans/README.md) still asserts "DECIDED: (A) positions-first (Option B rejected)"
...
- **2026-08-10 (consistency-pass)** — Full start-to-finish pass reconciling evolved decisions from review
  ...
  ([13](../streaming-compatibility-specs/13-tracker-to-canonical-mapping.md), [12](../streaming-compatibility-specs/12-standard-human-model.md)); **keep the parquet file**,
  migrate its schema to tidy-long ([10](../streaming-compatibility-specs/10-serialization-and-tidy-format.md)); **align, don't delete** the
  disabled `core/kinematics` (out of hot loops until validated — [06](../streaming-compatibility-specs/06-backend-refactor-and-cleanup.md));
...
- **2026-08-10 (standard-human)** — Confirmed: `bs/kinematics_core` **is** the rotation/kinematics engine
  ...
  [12 — Standard Human Model](../streaming-compatibility-specs/12-standard-human-model.md). **Open:** face blendshapes; offset magnitudes;
...
- **2026-08-10 (investigation)** — Investigated the overlap between SkellyModels' `Human` actor,
  ...
  SkellyForge parquet. Wrote [09](../streaming-compatibility-specs/09-standard-stream-protocol.md) (wire contract), [10](../streaming-compatibility-specs/10-serialization-and-tidy-format.md)
  (serialization), [11](../streaming-compatibility-specs/11-kinematics-fold-in.md) (kinematics fold-in). **Open:** confirm bs/ is the rotation
